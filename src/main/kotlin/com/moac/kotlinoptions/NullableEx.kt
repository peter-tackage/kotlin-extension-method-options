...
inline fun <T> T?.filter(filtering: (T) -> Boolean): T? {
    return this?.let { value -> if (filtering(value)) this else null }
}
...
inline fun <T : Any, R : Any> T?.combine(other: T?, bothSome: (T, T) -> R): R? {
    return this?.let { it1 -> other.map { it2 -> bothSome(it1, it2) } }
}
